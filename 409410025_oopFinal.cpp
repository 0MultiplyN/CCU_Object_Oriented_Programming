...
typedef pair<unsigned int,unsigned int> up ;
...
const uint INF = 0x3f3f3f3f; // infinite -> set margin ; 1061109567
uint numNodes = 0  ;
uint nodeCost = 0 ;
uint InsTime = 0 ,  UpdTime= 0  , SimDuration = 0 ;
uint SrcID = 0 , DstID = 0 , OldPathLen = 0 ;
uint LinkInNet = 0 ;
uint flagUp = 0 , flagIn = 0 , flagInIn = 0 , flagUpup = 0 ;

uint conID = 0 ; // controller ID
uint extra = 0 ;
uint waitd = 0 ;
uint oweight = 0 ;
vector<uint> ctrl_new_table ;
vector<uint> ctrl_upd_table ;
vector<uint> ctrl_del_table ;
vector<vector<uint>> consis_table(100) ;
...
class mynd{
    public :
        uint oact = INF;
        uint nact = INF ;
        uint ndpos = 0 ; // 0 for not in // 1 for origin in // 2 for origin in & upd
                         // 3 for origin in no change
                         // 4 for red point
                         // 5 for black point

        map< uint , uint >mapEntryOld ; // record old entry
        vector< uint > check ;
        map< uint , uint >mapEntryNew ; // record new entry
        map< uint , uint >mapLevel ;
        uint flagForNew = 1 ; // new entry has condition to print out ||  update or not  ;
        void constructOldTable( uint idxNode ) ; // by mapEntryOld using findEntryFuction
        void printOldTable( uint idxNode ) ; // cout old table answer
        void constructNewTable( uint idxNode ) ; // by mapEntryNew using findEntryFuction
        void printNewTable( uint idxNode ) ;
        uint level = 0 ;
};

mynd nd[ 10000 ] ; // for node int the internet
uint adjList[ 10000 ][ 10000 ] ; // record new weight
uint dis[ 10000 ][ 10000 ] ; // record distance by finding shortest path
uint nextStep[ 10000 ][ 10000 ] ; // next point by finding shortest path ;


//////////////////////////////////////////////////////

void findEntry( uint numNodes )
{
    uint i = 0 , j = 0 , k = 0 ;
    for ( k = 0 ; k < numNodes ; k++)
    {
        for ( i = 0; i < numNodes ; i++)
        {
            for ( j = 0; j < numNodes ; j++)
            {
                ...
                } // update path when  dis[ i ][ j ] > dis[ i ][ k ] + dis[ k ][ j ]
                else if( dis[ i ][ j ] == dis[ i ][ k ] + dis[ k ][ j ]  )
                {
                    if( nextStep[ i ][ j ] > nextStep[ i ][ k ]   )
                    {
                        nextStep[ i ][ j ] = nextStep[ i ][ k ] ;
                        // the same distance case for choosing smaller node ;
                    }
                    else continue ;
                }
                else continue ;
                // update shortest distance by Floyd Warshall Algorithm ;
                // and record step for build table
            }
        }
    }
}
...
void newrule( void )
{
    ...
    uint now = SrcID ;
    uint target = DstID ;
    while( now != target ) // my way src to dst
    {
        uint org = now ;
        now = nextStep[now][target] ;
        if( nd[org].ndpos == 0 ) // new
        {
            nd[ org ].nact = now ;
            nd[ org ].ndpos = 4 ;
            ctrl_new_table.push_back( org ) ;
        }
        else if( nd[ org ].ndpos == 1 ) // origin in
        {
            if( now != nd[ org ].oact )
            {
                nd[ org ].nact = now ;
                nd[ org ].ndpos = 2  ;
                ctrl_upd_table.push_back( org ) ;
            }
            else if( now == nd[ org ].oact ) // there is nothing to change still in 3
            {
                nd[ org ].ndpos = 3  ;
            }
        }
    ...
    for( uint i = 0 ; i < numNodes ; i++ )
    {
        ...
        {
            nd[ i ].ndpos == 5 ;
            ctrl_del_table.push_back( i ) ;
        }
    }
    uint a = ctrl_del_table.size( ) ;
    uint b = ctrl_upd_table.size( ) ;
    uint c = ctrl_new_table.size( ) ;
    extra += (a+b+c) * nodeCost ;
    ...
    while( now != target ) // my way src to dst
    {
        uint org = now ;
        now = nd[org].oact ;
        oweight += adjList[org][now] ;
    }
    if( oweight < extra ) waitd = 1 ;
...
void SDN_switch::recv_handler (packet *p){
    ...
    { // the switch receives a packet from the other switch
        ...
        //uint  pktID = p2->getPacketID( ) ;

        uint curID = getNodeID() ;
        uint destID = p2 -> getHeader()-> getDstID( ) ;
        //uint oldnxtID = p2 -> getHeader()->getNexID( );
        //cout << " node " << getNodeID() << " send the packet to " << destID << "\n";
        map<uint, uint>::iterator iter ;
        iter = routingtable.find( destID ) ;
        uint nxtID = 0 ;
        if( iter != routingtable.end() ) nxtID = iter->second ;
        else return ;
        p2->getHeader()->setPreID ( curID );
        p2->getHeader()->setNexID ( nxtID );
        //p2->getHeader()->setDstID ( destID );
        send_handler(p2);
    }
    ...
    { // the switch receives a packet from the controller
        ...
        //cout<<"5566\n" ;
        uint mat = l3->getMatID();
        uint act = l3->getActID();
        uint curID = getNodeID() ;
        //cout<<mat<<" "<<act<<" @"<<"\n" ;
        map<uint, uint>::iterator iter ;
        iter = routingtable.find( mat ) ;
        //uint nxtID = 0 ;
        if( iter != routingtable.end()) // put msg in routing table ;
        {
            routingtable[ mat ] = act  ;
        }
        else routingtable.insert(pair<uint, uint>( mat , act ) );
    ...
    else if( p->type() == "SDN_stat_packet"  )
    {
        ...
        uint curID = getNodeID() ;
        p4->getHeader()->setPreID ( curID ) ;
        p4->getHeader()->setNexID ( conID ) ;
        p4->getHeader()->setDstID( conID ) ;
        send_handler( p4 ) ;
    }
...
void SDN_controller::recv_handler (packet *p){
    ...
    { // the switch receives a packet from the controller
        ...
        SDN_ctrl_new_payload *l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_new_payload*> (p3->getPayload());
        uint dst = p3->getHeader()->getDstID ( ) ;
        //cout<<"5566\n" ;
        p3->getHeader()->setPreID ( conID ) ;
    ...
    else if( p->type() == "SDN_stat_packet"  )
    {
        ...
        if( statcnt == conID )
        {
            findEntry( numNodes ) ;
            newrule( ) ;
            uint len = ctrl_new_table.size( ) ;
            for( uint i = 0 ; i < len ; i ++ )
            {
                uint id = ctrl_new_table[i] ;
                ctrl_new_packet_event( conID , id , DstID , nextStep[id][DstID] ) ;
            }
...
int main()
{
    ...
    for( uint i = 0 ; i < numNodes ; i++ )
    {
        node::node_generator::generate("SDN_switch",i);
        node::id_to_node( i )->add_phy_neighbor( conID );
        node::id_to_node( conID )->add_phy_neighbor( i );
        for( uint j = 0 ; j < numNodes ; j++ )
        {
            adjList[ i ][ j ] = INF ;
            dis[ i ][ j ] = INF ;
        }
        // meanwhile process id
        // read the input and generate switch nodes
    }// initial adjacency list

    for( uint i = 0  ; i < OldPathLen ; i++ )
    {
        uint a = 0  , b = 0  , c = 0  ;
        cin>>a>>b>>c ;
        ...
        nd[b].oact = c ;
        nd[b].ndpos = 1 ; // origin in path

        ctrl_new_packet_event( conID , b , DstID , c , InsTime );
    }

    cin>>LinkInNet ;

    for( uint i = 0 ; i < LinkInNet ; i++ )//stat packet
    {
    ...
    for( uint i = 0 ; i < numNodes ; i++ )
    {
        map<unsigned int,unsigned int> Linktmp ;
        for( uint j = 0 ; j < numNodes ; j++ )
        {
            if( adjList[i][j] != INF )
            {
                Linktmp.insert(pair<uint, uint>( DstID , adjList[i][j] ) ) ;
                nextStep[ i ][ j ] = j ;
            }
            else nextStep[ i ][ j ] = INF ;
        }
        stat_packet_event( i , conID  , Linktmp , UpdTime ) ;
    }

    uint iisrc = 0  , iidst = 0 , iipktime = 0 ;
    cin>>iipktime>>iisrc>>iidst ;
    data_packet_event( iisrc , iidst , iipktime  ) ;
